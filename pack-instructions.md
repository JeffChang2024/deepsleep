...
> 静默日（过去24h无活跃对话）。延续自 YYYY-MM-DD 的未完成事项。连续静默 N 天。
...
连续静默不能无限搬运同样的内容。按连续静默天数分级处理：

| 连续静默天数 | 行为 |
|---|---|
| **1-3 天** | 完整 carry-forward：所有 Open Questions + 待办 + 行动项 + 各群进展标记 |
| **4-7 天** | 精简 carry-forward：只保留 Open Questions + 未完成待办。去掉行动项和群进展标记（已经不具时效性） |
| **8-14 天** | 最小 carry-forward：只保留 Open Questions。待办降级为"长期搁置"标记，不再每天搬运到 Today 段落 |
| **15+ 天** | 归档：将剩余 Open Questions 写入 `MEMORY.md` 的 `## Open Questions (Archived)` 段落（永久存储），daily file 只写一行 `> 长期静默（N天），活跃事项已归档至 MEMORY.md`。**不再生成群级段落，不再触发 dispatch 发送晨报。** |
...
- 如果之前已归档到 MEMORY.md → Phase 3 加载 MEMORY.md 的 Archived Open Questions 段落可恢复上下文
- 如果静默 < 15 天 → 快照文件仍有最新 carry-forward 内容，Phase 3 正常加载

⚠️ **这一步是防止连续静默天导致记忆断裂的关键**。没有它，3天不说话 = 快照过期 = 滚动合并误杀所有条目。衰减机制确保不会 Day 50 还在搬运完全一样的内容。
