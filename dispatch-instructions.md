...
3. **Merge 规则**：
   - 保留最近 3 天的条目（按 `[日期]` 前缀判断）
   - 超过 3 天的旧条目 → 删除（除非是 Open Question 或未完成的待办）
   - 昨天的新内容 → 追加到"最近进展"
   - Open Questions：保留所有未解决的（不受 3 天限制），已解决的删除
   - 待办：保留所有未勾选的 `- [ ]`，已完成的 `- [x]` 超过 3 天则删除
4. **写入合并后的快照**
