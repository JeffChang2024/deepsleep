...
### ⚠️ 静默日保护规则 + 衰减
如果昨天的 daily summary 标记为静默日（包含"静默日"或"当日无新对话"），按连续静默天数分级处理：

**1-14 天静默**：
- **不清除现有快照中的进展条目**
- **只更新快照的时间戳**（`> 更新时间：YYYY-MM-DD 00:10`）
- 保留所有 Open Questions 和未完成待办
- 快照末尾标注 `> 连续静默 N 天，最后活跃：YYYY-MM-DD`

**15+ 天静默**：
- Open Questions 已归档到 MEMORY.md（pack 阶段处理）
- **不再更新快照文件**（避免无意义重写）
- **不发送晨报**（没有新内容可报）
- 快照保持冻结状态，等待下次活跃时由新的 dispatch 刷新

**恢复活跃时**：
- 正常 merge：新的活跃内容写入快照，替代旧的 carry-forward 条目
- 如果有 Archived Open Questions 在 MEMORY.md 中，Phase 3 加载时会自动获取（因为 agent 的 AGENTS.md 要求主 session 加载 MEMORY.md）
